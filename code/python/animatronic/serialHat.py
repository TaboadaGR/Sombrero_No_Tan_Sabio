...
def open_serial_port(com_port, baudrate):
    ...
    try:
        ...
        logger.info(f"Puerto serie '{com_port}' abierto correctamente.")
        print(f"Puerto serie '{com_port}' abierto correctamente.")
        return ser
    except serial.SerialException as e:
        logger.error(f"Error al abrir el puerto '{com_port}': {e}")
...
def send_servo_command(ser_port, command):
    ...
    if ser_port and ser_port.is_open:
        try:
            ...
            logger.debug(f"Enviado a Arduino: {message.strip()}") # strip() para no loguear el '\n'
            if command == '1':
                print("Servo: MOVING")
            elif command == '0':
                print("Servo: STOPPED")
            ...
            logger.error(f"Error al enviar comando '{command}' a Arduino: {e}", exc_info=True)
            print(f"Error crítico de comunicación con Arduino: {e}")
            return False
    else:
        logger.warning(f"Intento de enviar comando '{command}', pero el puerto serial no está abierto.")
        return False
    ...
    if ser_port and ser_port.is_open and ser_port.in_waiting > 0:
        try:
            ...
            if feedback:
                ...
                logger.debug(f"Recibido de Arduino: {feedback}")
        except Exception as e:
            logger.error(f"Error al leer feedback de Arduino: {e}", exc_info=True)
